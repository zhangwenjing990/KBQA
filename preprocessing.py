...
def countChar():
    # 统计字符个数
    fi = open('./data/nlpcc-iccpol-2016.kbqa.training-data', 'r', encoding='utf8')
...
def addAliasForKB(kbDictRaw):
    ...
    patternSub = re.compile(
        r'(\s*\(.*\)\s*)|(\s*（.*）\s*)')  # subject需按照 subject (Description) || Predicate || Object 的方式抽取, 其中(Description)可选
...
def convertToJson(inputPath='./data/vec_zhwiki_300mc20.txt', outputPath='vectorJson.utf8' \
                  , encode='utf8'):
...
# 用训练数据训练答案模板
def getAnswerPatten(inputPath='./data/nlpcc-iccpol-2016.kbqa.training-data', outputPath='outputAP'):
    inputEncoding = 'utf8'
    outputEncoding = 'utf8'
    ...
    pattern = re.compile(
        r'[·•\-\s]|(\[[0-9]*\])')  # pattern to clean predicate, in order to be consistent with KB clean method
...
if __name__ == '__main__':
    ...
    print('Cleaning kb......')
    kbDictRaw = loadKB('./data/nlpcc-iccpol-2016.kbqa.kb')
    addAliasForKB(kbDictRaw)
    print('\nDone!')
